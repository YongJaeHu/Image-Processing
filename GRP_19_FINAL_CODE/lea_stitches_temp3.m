function dirty_temp_match = lea_stitches_temp3(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp3=imread('leather_glove_stitches_temp3.jpg');%part of image
[xt,yt,zt]=size(tp3);
if zt==1
    temp3=tp3;
else
...
for k=1:n
    ...
    glove_img(i(k):i(k)+xt,j(k))=1;glove_img(i(k):2:i(k)+xt,j(k))=0;
     ...
     display('Nope')
     msgbox('No, It is not Stitches Defect.', 'Error', 'error');
end;  
