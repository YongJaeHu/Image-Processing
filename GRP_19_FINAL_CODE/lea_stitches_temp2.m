function dirty_temp_match = lea_stitches_temp2(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp2=imread('leather_glove_stitches_temp2.jpg');%part of image
[xt,yt,zt]=size(tp2);
if zt==1
    temp2=tp2;
else
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp2(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
