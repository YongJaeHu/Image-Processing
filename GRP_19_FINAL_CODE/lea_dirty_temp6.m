function dirty_temp_match = lea_dirty_temp6(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp6=imread('leather_glove_dirty_temp6.jpg');%part of image
[xt,yt,zt]=size(tp6);
if zt==1
    temp6=tp6;
else
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp6(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
