function hole_temp_match = lea_tearfin_temp4(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp4=imread('leather_glove_tearfin_temp4.jpg');%part of image
[xt,yt,zt]=size(tp4);
if zt==1
    temp4=tp4;
else
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp4(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
