function hole_temp_match = lea_tearfin_temp5(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp5=imread('leather_glove_tearfin_temp5.jpg');%part of image
[xt,yt,zt]=size(tp5);
if zt==1
    temp5=tp5;
else
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp5(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
...
for k=1:n
    ...
    display('Nope')
    msgbox('No, It is not Tearing on Fingertips Defect.', 'Error', 'error');
end;  
