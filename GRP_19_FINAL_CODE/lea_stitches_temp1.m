function stitches_temp_match = lea_stitches_temp1(glove)
% glove=imread('leather_glove_stitches1.jpg');%main image
[xi,yi,zi]=size(glove);
if zi==1
    img1=glove;
else
...
tp1=imread('leather_glove_stitches_temp1.jpg');%part of image
[xt,yt,zt]=size(tp1);
if zt==1
    temp1=tp1;
else
...
if(min(min(c))<th)% for big or noisy images can keep threshold 50;
    display('Stitches Defect is found in given picture')
    msgbox('Yes, It is Stitches Defect', 'Error', 'error');
    [r1,c1]=size(c);
n=0;
clear i;clear j;
for i1=1:r1
