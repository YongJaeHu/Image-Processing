function dirty_temp_match = lea_dirty_temp11(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp11=imread('leather_glove_dirty_temp7.jpg');%part of image
[xt,yt,zt]=size(tp11);
if zt==1
    temp11=tp11;
else
temp11=rgb2gray(tp11);
end;
[xt,yt,zt]=size(temp11);
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp11(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
