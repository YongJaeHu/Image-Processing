function dirty_temp_match = lea_dirty_temp8(glove)
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp8=imread('leather_glove_dirty_temp7.jpg');%part of image
[xt,yt,zt]=size(tp8);
if zt==1
    temp8=tp8;
else
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp8(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
