function tearfin_temp_match = lea_tearfin_temp1(glove)
% glove=imread('leather_glove_tearing_fingertips1.jpg');%main image
[xi,yi,zi]=size(glove);
if zi==1
    glove_img=glove;
else
...
tp1=imread('leather_glove_tearfin_temp1.jpg');%part of image
[xt,yt,zt]=size(tp1);
if zt==1
    temp1=tp1;
else
...
for i=1:(xi-xt);
    for j=1:(yi-yt);
        ...
        for k=1:xt;
            for l=1:yt;
                a=abs(double(glove_img(i+k,j+l))-double(temp1(k,l)));
                b=b+a;
            end
        end
        c(i,j)=b;
